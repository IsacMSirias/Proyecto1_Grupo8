...
El proyecto presente consta en el diseño solución e implementación de una interfaz gráfica, sockets y el uso de una lista doblemente enlazada, la que al ser una estructura de datos, ayudará a almacenar información necesaria que se utilizará para la realización del juego de mesa multijudor Socket Math, el cual se trata de un juego, que tendrá como principal objetivo, resolver operaciones matemáticas simples hasta llegar al final del tablero; el cual está desarrollado en el lenguaje de programación Java (jdk 16.0).
...
Una vez los dos jugadores estén en una misma partida se desplegará la pantalla de juego, la cual contará con un tablero y además de esto en pantalla se podrá ver el progreso de cada uno de los jugadores (Su respectiva posición en el tablero y en la casilla que se encuentra). Sin contar que está pantalla también cuenta con un dado, el cual genera números random en un rango de 1 a 3, dependiendo de que numero salga en el dado, el jugador 1 o el jugador 2, dependiendo del turno se moverá n casillas hacía adelante o hacía atrás.

Cada casilla presentará un atributo de tres que ya están establecidos, correspondientes a la casilla de reto, trampa y reto. 


* **Casilla de reto**

Esta casilla es una de las más comunes dentro del tablero, su propósito es ponerle un reto matemático al otro jugador. si este falla su respuesta se devolverá una casilla atrás, en caso de acierte, mantendrá su posición actual, pero independientemente si el jugador en cuestión falla o acierta la pregunta, el otro jugador avanzará una casilla.
...
Al caer en esta casilla, esta permite que el jugador pueda avanzar entre una y tres casillas hacia adelante, el movimiento dependerá del numero random que se genere en ese momento.

* **Casilla trampa**

Esta casilla funciona de la misma manera que la de túnel, pero en vez de avanzar, se retrocede en las casillas del tablero

## Diagrama de clases

![UMLDiagram](https://user-images.githubusercontent.com/83249731/136306820-d98ff423-b281-409a-b4fc-d379f80b519b.png)
...
Se tuvo que exportar todo el proyecto a un IDE con una extensión diferente, dado que cuando se empezó se guardó en un JAVA proporcionado por IntelliJ, pero mas tarde el grupo de trabajo se dio cuenta de que para poder pasar objetos por Json, se necesitaban usar dependencias propias de un proyecto con extensión MAVEN, por lo que eso hizo que la elaboración del proyecto se atrasara considerablemente.
## Conclusiones
